 ...
 * - Castle Fantasy (Sky) canlı duvar kağıdının GPU üzerindeki piksel renklendirme (fragment shader) işlemlerini gerçekleştirir.
 * - Zaman, çözünürlük, gün ilerlemesi (day progress), güneş/ay konumları ve parallax (kaydırma) bilgilerini alarak dinamik bir gökyüzü çizer.
 * 
 * NE İŞ YAPMASI GEREKİYOR (MİMARİ HEDEF):
 * - Ekranda kale, bulutlar ve güneş/ay dokularını (textures) üst üste düzgün katmanlar halinde harmanlayarak (blend) çizmelidir.
 * - Parallax etkisinin çalışabilmesi için cihazın hareketine göre motor tarafından beslenen "u_Parallax" koordinat offsetlerini kullanmalıdır.
 * 
 * HEDEF:
 * - Castle Fantasy sahnesini, günün saatlerine ve cihaz hareketlerine göre kesintisiz, akıcı ve güvenilir bir parallax 3D hissiyle ekrana yansıtmaktır.
 */

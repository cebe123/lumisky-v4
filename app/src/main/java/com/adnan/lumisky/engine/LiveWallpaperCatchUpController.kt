...
class LiveWallpaperCatchUpController(
    ...
    fun resolveWindow(
        ...
        val sunrise = sunriseMinute.coerceIn(0, MINUTES_PER_DAY) / MINUTES_PER_DAY.toFloat()
        val sunset = sunsetMinute.coerceIn(0, MINUTES_PER_DAY) / MINUTES_PER_DAY.toFloat()
        ...
        val start = if (isDaytime) sunrise else sunset
        val target = when {
            isDaytime -> if (now >= start) now else now + 1f
            now >= start -> now
            else -> now + 1f
        }

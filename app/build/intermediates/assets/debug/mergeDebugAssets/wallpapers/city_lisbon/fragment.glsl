...
float circleMask(vec2 uv, vec2 center, float radius) {
	...
	return 1.0 - smoothstep(radius * 0.82, radius, length(delta));
}
...
float getStars(vec2 uv) {
	...
	float core = 1.0 - smoothstep(0.0, 0.12, length(delta));
	float cross = max(
		1.0 - smoothstep(0.0, 0.055, abs(delta.x)) * 1.0 - smoothstep(0.0, 0.18, abs(delta.y)),
		1.0 - smoothstep(0.0, 0.055, abs(delta.y)) * 1.0 - smoothstep(0.0, 0.18, abs(delta.x))
	);
	float shape = max(core, cross * 0.42);
	float microStars = step(0.88, hash(starCell + vec2(43.0, 59.0))) *
		(1.0 - smoothstep(0.0, 0.07, length(delta))) * 0.34;
...
vec4 sampleForegroundPreservingColor(vec2 uv) {
    ...
    // FAST PATH: If the pixel is fully transparent or fully opaque, skip the expensive 7-tap blur
    if (fg.a < 0.05 || fg.a > 0.95) {
        return fg;
    }
...
void main() {
	...
	float starVisibility = smoothstep(skyHorizonY + 0.01, skyHorizonY + 0.26, skyUv.y);
        float stars = 0.0;
        if (u_HasStars > 0.0 && u_NightAmount > 0.0) {
            stars = getStars(skyUv);
        }
        sky += vec3(0.96, 0.98, 1.0) * stars * u_NightAmount * starVisibility * 1.42;
    ...
    if (u_IsNight > 0.5) {
        ...
        // Only compute expensive craters/surface if within the moon's radius + slight margin
        if (moonDist < MOON_RADIUS + 0.015) {

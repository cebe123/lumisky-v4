...
void main() {
    ...
    if (uv.y >= HORIZON_Y) {
        ...
        // Clouds (Optimized FBM only when above horizon and visible)
        float cloudMask = 0.0;
        float finalCloudAlpha = max(u_CloudAlpha, 0.24);
        if (uv.y > HORIZON_Y && finalCloudAlpha > 0.0) {
            vec2 cloudUV = uv;
